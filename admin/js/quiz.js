(function($) {
	...
	$(document).ready(function() {
		...
		var lesson_id = parseInt(document.getElementById('ibedu-quiz-lesson-id').value, 10);
		var questionType = $('#ibedu-question-type');
		...
		var MultipleChoiceAnswerView = Backbone.View.extend({
			className: 'answer',
			tagName: 'tr',
			template: _.template($('#tpl-ibedu-multichoiceanswer').html()),
			questionModel: null,
		...
		var QuestionView = Backbone.View.extend({
			...
			hideMessage: function(cb) {
				this.$el.find('.ibedu-overlay').fadeOut(200, function() {
					$(this).remove();
		...
		var MultipleChoiceQuestionView = QuestionView.extend({
			...
			template: _.template($('#tpl-ibedu-multiplechoicequestion').html()),
		...
		var WrittenAnswerQuestionView = QuestionView.extend({
			...
			template: _.template($('#tpl-ibedu-writtenanswerquestion').html()),
		...
		var QuizView = Backbone.View.extend({
			...
			renderQuestion: function(question) {
				...
				view.render();
				$('#ibedu-questions').append(view.el);
			},
			...
			updateQuestionsOrder: function() {
				...
				// Disable sortable.
				$('#ibedu-questions').sortable('disable');
				...
				$.ajax({
					...
					complete: function() {
						$('#ibedu-questions').sortable('enable');
					}
				});
			}
		});
