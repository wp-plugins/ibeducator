...
<?php
	...
	if ( 'payment-cancelled' === $message ) {
		echo '<div class="ibedu-message success">' . __( 'Payment has been cancelled.', 'ibeducator' ) . '</div>';
	}
	...
	if ( $courses || $pending_courses ) {
		if ( $pending_courses ) {
			...
			echo '<table class="ibedu-courses ibedu-courses-pending">';
		...
		if ( $courses && $courses['entries'] ) {
			...
			if ( array_key_exists( 'inprogress', $courses['statuses'] ) ) {
				...
				echo '<table class="ibedu-courses ibedu-courses-inprogress">';
			...
			if ( array_key_exists( 'complete', $courses['statuses'] ) ) {
				...
				echo '<table class="ibedu-courses ibedu-courses-complete">';
