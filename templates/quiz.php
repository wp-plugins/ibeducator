...
<?php if ( $questions ) : ?>
	<?php
		...
		if ( $message ) {
			switch ( $message ) {
				case 'empty-answers':
					echo '<div class="ibedu-message error">' . __( 'Please answer all questions before submitting the quiz.', 'ibeducator' ) . '</div>';
					break;

				case 'quiz-submitted':
					echo '<div class="ibedu-message success">' . __( 'Thank you. The quiz has been accepted.', 'ibeducator' ) . '</div>';
					break;
			}
		}
	...
	<section id="ibedu-quiz" class="<?php echo ( $quiz_submitted ) ? 'ibedu-quiz-complete' : 'ibedu-quiz-inprogress'; ?>">
		<?php if ( $quiz_submitted ) : ?>
		<section class="ibedu-quiz-grade">
		...
		<form id="ibedu-quiz-form" class="ibedu-form" method="post" action="<?php echo esc_url( ibedu_endpoint_url( 'edu-action', 'submit-quiz', get_permalink() ) ); ?>">
			...
			<?php
				...
				foreach ( $questions as $question ) {
					if ( 'multiplechoice' == $question->question_type ) {
						...
						echo '<div class="ibedu-question">';
						echo '<div class="label">' . esc_html( $question->question ) . '</div>';
						echo '<ul class="ibedu-answers">' . $answers_html . '</ul></div>';
						...
						if ( ! $quiz_submitted ) {
							...
							echo '<div class="ibedu-question-answer">'
							...
							if ( $user_answer ) {
								echo '<div class="ibedu-question-answer">' . esc_html( $user_answer->answer_text ) . '</div>';
							}
						}
			...
			<?php if ( ! $quiz_submitted ) : ?>
				<div class="ibedu-buttons">
					<button class="ibedu-button" type="submit"><?php _e( 'Submit', 'ibeducator' ); ?></button>
				</div>
			<?php endif; ?>
		</form>
		<?php //endif; ?>
