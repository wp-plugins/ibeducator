...
<article id="course-<?php the_ID(); ?>" <?php post_class( 'ibedu-course-single' ); ?>>
	<?php if ( apply_filters( 'ibedu_show_single_course_title', true ) ) : ?>
			...
			switch ( $status ) {
				...
				default:
					...
					<div class="ibedu-course-price">
						<span class="price"><?php echo ( 0 == $price ) ? __( 'Free', 'ibeducator' ) : ibedu_format_price( $price ); ?></span>
						<a href="<?php echo esc_url( ibedu_endpoint_url( 'edu-course', get_the_ID(), get_permalink( ibedu_page_id( 'payment' ) ) ) ); ?>" class="ibedu-button"><?php _e( 'Register', 'ibeducator' ); ?></a>
					</div>
					<?php
			}
		?>
